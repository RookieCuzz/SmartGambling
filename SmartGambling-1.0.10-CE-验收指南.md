...
- 两名玩家使用相同买入，账本必须出现两笔 STAKE 和两笔原子 LOCK。
- 确认桌主/小盲翻牌前先行动，对手/大盲在翻牌、转牌、河牌各圈先行动。
- 分别验证过牌、跟注、最小加注、底池加注、弃牌和全下；非法越序操作不得生效。
- 摊牌验证同花顺至高牌九类比较、A2345 顺子、平分底池和全下后的自动跑牌。
- 主动离桌按弃牌结算；座椅/开局技术故障必须退还两笔完整买入。
- 最终两名玩家到账总额必须等于两笔买入总和，不能丢失未投入底池的桌面筹码。
...
最新干净构建已通过 27 个测试类、121 个测试（0 failure/error/skipped）。隔离
Paper 测试服上的 Mineflayer 协议级脚本已覆盖五玩法、dataVersion 3 干净重启及
